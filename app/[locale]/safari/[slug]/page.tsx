...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = month 
    ? `${park} Safari in ${month.charAt(0).toUpperCase() + month.slice(1)} 2026 | Asili Yetu`
    : `${park} Safari Expeditions | Asili Yetu`;
...
export default async function SafariDirectoryPage({ params }: Props) {
  ...
  const breadcrumbItems = [
    { name: "Home", item: "/" },
    { name: "Safaris", item: "/packages" },
    { name: `${parkQuery} Safari`, item: `/safari/${slug}` }
  ];

  const itemListSchema = {
    "@type": "ItemList",
    "name": `${parkQuery} Safari Packages`,
  ...
  return (
    <div className="min-h-screen pt-32 pb-20 px-6 bg-background">
      <StructuredData type="BreadcrumbList" data={getBreadcrumbSchema(baseUrl, breadcrumbItems)} />
